...
class ViewTest(BaseTest):
    ...
    def check_folder(self, folder):
        ...
        self.assertRedirects(response, "{0}?{1}={2}".format(settings.LOGIN_URL, REDIRECT_FIELD_NAME, url), fetch_redirect_response=False)
        # authenticated
        self.assertTrue(self.client.login(username='foo', password='pass'))
        response = self.client.get(url)
    ...
    def test_write_recipient(self):
        ...
        url = reverse('postman:write', args=[str(self.user1.pk) + ':' + str(self.user2.pk)])
        response = self.client.get(url)
        self.assertContains(response, 'value="{}, {}"'.format(self.user1.pk, self.user2.pk))
...
class FieldTest(BaseTest):
    ...
    def test_clean(self):
        ...
        self.assertEqual(f.clean(str(self.user2.pk)), [self.user2])
...
class MessageManagerTest(BaseTest):
    ...
    def test_query_compiler(self):
        ...
        self.assertListEqual(list(qs.filter(recipient_id=self.user2.pk)), [m])  # param 2, must stay at the end
...
class ContextProcessorsTest(BaseTest):
    ...
    def _check_inbox(self, urlname, num_extra_queries, value=''):
        ...
        with self.assertNumQueries(num_extra_queries):
            response = self.client.get(url)
    ...
    def test_inbox(self):
        ...
        self.assertTrue(self.client.login(username='foo', password='pass'))
        self.check_inbox_without_eval(1)
        self.check_inbox_with_eval(1, '0')
        ...
        self.check_inbox_without_eval(1)
        self.check_inbox_with_eval(1, '1')
...
class UtilsTest(BaseTest):
    ...
    def test_get_user_name(self):
        ...
        self.assertEqual(get_user_name(self.user1), str(self.user1.pk)) 
