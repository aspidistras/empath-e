"""Script for adding disorders data to db"""
...
def get_disorders_data():
    """Browse JSON file with disorders data to add to db"""

    with open("app/data/disorders.json") as json_disorders:
        disorders_data = json.load(json_disorders)
        for disorder in disorders_data:
            try:
                disorder_content = Disorder.objects.create(details=disorder["content"],
                                                           name=disorder["name"],
                                                           url_pattern=disorder["url_pattern"])
            except:
