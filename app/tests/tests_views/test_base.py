...
class BasicViewsTestCase(TestCase):
    ...
    def setUp(self):
        self.disorder = Disorder.objects.create(name='test',
                                                details='test data', url_pattern='test')
    ...
    def test_contat_form(self):
        ...
        response = self.client.post('/', {'email': 'test@test.fr',
                                          'subject': 'test', 'message': 'test'})
        self.assertEqual(len(mail.outbox), 1)
        self.assertEqual(mail.outbox[0].subject, 'test')
        self.assertEqual(mail.outbox[0].body,
                         'Message suivant : \'test\' de la part de test@test.fr')
    ...
    def test_disorders_list_page(self):
        """Check that disorders list page returns status code 200
        and displays disorders list"""
    ...
    def test_disorder_details_page(self):
        """Check that disorder detail page returns status code 200
        and displays disorder data"""

        response = self.client.get(reverse('app:disorder_details',
                                           args=[self.disorder.url_pattern]))
